/**
La plataforma UTN-eLearning brinda capacitaciones virtuales, las cuales son dictadas por
profesores de las distintas regionales de la UTN. Para el seguimiento de los mismos registra
las inscripciones a los cursos en un archivo llamado INSCRIPCIONES. El archivo está
ordenado por Facultad, Tema, Curso, Turno, fecha inicio e ID curso. Además, contiene la
duración del curso en horas (es un entero), el cupo (cantidad de alumnos que puede tener
como máximo) y la cantidad de inscriptos actuales.

|- Facultad -|- Tema -|- Curso -|- Turno -|- fecha_inicio -|- ID_curso -| Duración | Cupo | Cant Inscriptos |

1. Obtener totales parciales de cantidad de inscriptos por Facultad, tema y curso
2. Mostrar el total general de inscriptos
*/

Accion consigna1 Es
Ambiente

  FECHA = REGISTRO
    aa: N(4)
    mm: 1..12
    dd: 1.31
  FinRegistro

  INSCRIPCIONES = REGISTRO
    Clave = REGISTRO
      Facultad: AN(20)
      Tema: AN(10)
      Curso: AN(10)
      Turno: AN(10)
      fecha_inicio: FECHA
      ID_curso: AN(20)
    FinRegistro
      Duración: AN(20)
      Cupo: AN(20)
      Cant_Inscriptos: AN(20)
  FinRegistro

  Archivo1: Archivo ordenado por Clave
  reg1: INSCRIPCIONES
  
  tot_fac, tot_tema, tot_curso: entero
  tot_global: entero

  res_fac: AN(20)
  res_tema: AN(10)
  res_curso: AN(10)
  
  Subaccion corte_curso Es
    Esc("La cantidad de inscriptos por curso", res_curso "Total", tot_curso);
    tot_tema := tot_tema + tot_curso
    res_curso := reg1.Clave.Curso
    tot_curso := 0
  FinSub
  
  Subaccion corte_tema Es
    corte_curso()
    Esc("La cantidad de inscriptos por tema", res_tema "Total", tot_tema);
    tot_fac := tot_fac + tot_global
    res_tema := reg1.Clave.Tema
    tot_global := 0
  FinSub

  Subaccion corte_facultad Es
    corte_tema()
    Esc("La cantidad de inscriptos por facultad", res_fac "Total", tot_fac);
    tot_global := tot_global + tot_fac
    res_fac := reg1.Clave.Facultad
    tot_fac := 0
  FinSub
  
Proceso
  Abrir E/(Archivo1)
  Leer(Archivo1,reg1)
  
  tot_fac := 0 
  tot_tema := 0 
  tot_curso := 0
  tot_global := 0

  res_fac := reg1.Clave.Facultad
  res_tema := reg1.Clave.Tema
  res_curso := reg1.Clave.Curso

  Mientras NFDA(Archivo1) Hacer
    Si (res_fac <> reg1.Clave.Facultad) Entonces
      corte_facultad()
        sino
          Si (res_tema <> reg1.Clave.Tema) Entonces
            corte_tema()
            sino
              Si (res_curso <> reg1.Clave.Curso) Entonces
                corte_curso()
              FinSi  
          FinSi
    FinSi
    
    tot_curso := tot_curso + reg1.Cant_Inscriptos
    Leer(Archivo1,reg1)
  FinMientras
  corte_facultad()
  Cerra(Archivo1)
  Esc("El total de alumnos inscriptos es de", tot_global)

FinAccion  
